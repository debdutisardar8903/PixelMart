...
function SignupPasswordForm() {
  ...
  useEffect(() => {
    ...
    if (!nameParam || !emailParam) {
      // If no name or email, redirect back to signup page
  ...
  const handleSignup = async (e) => {
    ...
    try {
      // Register using Firebase auth with display name
      ...
      if (errorCode === 'email-already-in-use' || error.message?.includes('email-already-in-use')) {
        errorMessage = 'An account with this email already exists.';
      } else if (errorCode === 'invalid-email' || error.message?.includes('invalid-email')) {
        errorMessage = 'Invalid email address.';
      } else if (errorCode === 'weak-password' || error.message?.includes('weak-password')) {
        errorMessage = 'Password is too weak. Please choose a stronger password.';
      } else if (errorCode === 'operation-not-allowed' || error.message?.includes('operation-not-allowed')) {
        errorMessage = 'Email/password accounts are not enabled.';
      } else if (error.message && error.message !== 'Signup failed. Please try again.') {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm/6 text-gray-600 tracking-[-.01em]">
            Almost done, <span className="font-medium">{name}</span>
          </p>
        </div>
