...
function SignupForm() {
  ...
  const handleContinue = async (e) => {
    ...
    // Navigate to password page with name and email
    router.push(`/auth/signup/password?name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&redirect=${encodeURIComponent(redirect)}`);
    setIsLoading(false);
  };
    ...
    <div className="font-sans min-h-screen flex items-center justify-center p-8 bg-white">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          />
          <h1 className="text-2xl font-semibold tracking-[-.01em] mb-2">Create your account</h1>
        ...
        <form onSubmit={handleContinue} className="space-y-6">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-2 tracking-[-.01em]">
              Full Name
            </label>
            <input
              id="name"
        ...
        <div className="mt-8 text-center">
          <p className="text-sm/6 text-gray-600 tracking-[-.01em]">
            ...
            <button 
              onClick={() => router.push(`/auth?redirect=${encodeURIComponent(redirect)}`)}
