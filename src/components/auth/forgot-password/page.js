...
function ForgotPasswordForm() {
  ...
  const { resetPassword, loading: authLoading } = authContext || { resetPassword: null, loading: true };
  
  // Debug auth context
  console.log('Auth context in forgot password:', { 
    authContext: !!authContext, 
    resetPassword: !!resetPassword,
    authLoading,
    user: authContext?.user?.email,
    allContextKeys: authContext ? Object.keys(authContext) : []
  });
  ...
  const handleResetPassword = async (e) => {
    e.preventDefault();
    console.log('Reset password clicked', { email, resetPassword: !!resetPassword });
    ...
    if (authLoading) {
      console.log('Auth context still loading, please wait...');
      setErrorMessage('Loading authentication... Please wait.');
      return;
    }
    
    if (!resetPassword) {
      console.error('resetPassword function not available');
      console.error('Available auth functions:', authContext ? Object.keys(authContext) : 'No auth context');
      setErrorMessage('Password reset is not available. Please try again later.');
      return;
    }
    ...
    try {
      ...
      // Check if it's a Firebase error or our custom error
      const errorCode = error.code || (error.message && error.message.includes('auth/') ? error.message.split('auth/')[1]?.split(' ')[0] : null);
      
      if (errorCode === 'user-not-found' || error.message?.includes('user-not-found')) {
        errorMsg = 'No account found with this email address.';
      } else if (errorCode === 'invalid-email' || error.message?.includes('invalid-email')) {
        errorMsg = 'Invalid email address.';
      } else if (errorCode === 'too-many-requests' || error.message?.includes('too-many-requests')) {
        errorMsg = 'Too many requests. Please try again later.';
      } else if (error.message && error.message !== 'Failed to send reset email. Please try again.') {
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleResetPassword} className="space-y-6">
          ...
          <button
            ...
            onClick={(e) => {
              console.log('Button clicked', { email, isLoading, disabled: !email || isLoading });
            }}
            className="w-full rounded-full border border-solid border-transparent transition-colors flex items-center justify-center bg-black text-white gap-2 hover:bg-gray-800 font-medium text-sm sm:text-base h-12 px-5 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <>
