    ...
    const fetchBanners = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          // Check for video banner in banners/video structure
          if (bannersData.video && bannersData.video.url) {
            setBannerUrl(bannersData.video.url);
            console.log('Video banner URL loaded from banners/video:', bannersData.video.url);
            return;
          }
          
          // Check for multiple banners structure - find video banners
          ...
          for (const [key, banner] of bannerEntries) {
            if (banner && typeof banner === 'object' && banner.url) {
              ...
              if (isVideo) {
                ...
                console.log('Video banner URL loaded from banners/' + key + ':', banner.url);
                return;
              }
            }
          }
